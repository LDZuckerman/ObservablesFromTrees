...
from accelerate import Accelerator # [!!] PUT THIS BACK ONCE ON GPU
accelerator = Accelerator() # [!!] PUT THIS BACK ONCE ON GPU
device = accelerator.device # [!!] PUT THIS BACK ONCE ON GPU
...
def load_data(data_path, data_file, targets, del_feats, scale, test=0, splits=[70, 10, 20], ): ##data_params
    datat=pickle.load(open(osp.expanduser(f'{os.getcwd()}/{data_path}/{data_file}'), 'rb'))
    a=np.arange(43)
    ...
    testidx_file = osp.expanduser(f'{os.getcwd()}/{data_path}/{data_file.replace(".pkl", "")}_testidx{splits[2]}.npy')
    ...
    if test: # if "test" run, which Christian uses to mean training the same model again completely from scratch on train+val, then testing on test
        ...
        return trainval_data[:int(len(trainval_data)*(1-val_pct))], trainval_data[int(len(trainval_data)*(val_pct)):] #train, val # 70%/10% of total
...
def test(loader, model, targs, l_func, scale):
    ...
    with torch.no_grad(): ##this solves it!!! LOL solves what?
        for data in loader: 
...
def train_model(construct_dict):
    ...
    out_pointer=osp.expanduser(out_dir) # Expanduser changes path type? Will cause issues on local pc?
    ...
    try:  ##this is a bad solution but it works [?] Bad soln to what?
    ...
    run_name = f'{construct_dict["id"]}_{construct_dict["model"]}'   # run_name = construct_dict['model']+f'_{case}'+f'_{make_id()}'
    for trial in range(n_trials):
        ...
        if save:  # Make folder for saved states
            ...
            if not osp.isdir(model_path):
                ...
                print(f'Made folder {model_path} for saving model')
        ...
        _, _, test_loader = accelerator.prepare(model, optimizer, test_loader) # [!!] PUT THIS BACK ONCE ON GPU
        model, optimizer, train_loader = accelerator.prepare(model, optimizer, train_loader) # [!!] PUT THIS BACK ONCE ON GPU
        ...
        def train(epoch, schedule):
            model.train()
            return_loss=0
            er_loss = torch.cuda.FloatTensor([0]) # er_loss = torch.FloatTensor([0]) #  [!!] CHANGE BACK ONCE ON GPU
            si_loss = torch.cuda.FloatTensor([0]) # si_loss = torch.FloatTensor([0]) # [!!] CHANGE BACK ONCE ON GPU
            rh_loss = torch.cuda.FloatTensor([0]) # rh_loss = torch.FloatTensor([0]) # [!!] CHANGE BACK ONCE ON GPU
            for data in train_loader: 
                # data = data.to('cpu') # [!!] DELETE ONCE ON GPU
                ...
                return_loss+=loss
                accelerator.backward(loss)  # loss.backward() # [!!] CHANGE THIS BACK ONCE ON GPU
                optimizer.step() 
                optimizer.zero_grad()
        ...
        for epoch in tqdm(range(n_epochs)):
            ...
            if (epoch+1)%val_epoch==0: # If this is an epoch we want to validate at too
        ...
        # before adding hparams to write, must change any lists to string
        from six import string_types
        for k, v in paramsf.items():
